...
if __name__ == '__main__':
    ...
    # _temp1 = training_temp.sample(n=6, random_state=None, replace=False)
    # _temp2 = _temp1.index
    # _temp3 = _temp2[0]
    # Question6
    # fig, ax = plt.subplots(3, 3, figsize=(10, 10))
    ...
    #         _training = training_temp
    #         _training_index = _training.index
    #         _training = _training.values
    #         for _ in range(epoch_times):
    #             y_i = w[0] + w[1] * _training[index][0] + w[2] * _training[index][1] + w[3] * _training[index][2]
    #             _mean_loss = (y_i - training_price[index]) / (
    #                     2 * np.sqrt(np.square(y_i - training_price[index])) + 4)
    #             w[0] = w[0] - alphas[i] * 1 * _mean_loss
    #             w[1] = w[1] - alphas[i] * y_values[index][0] * _mean_loss
    #             w[2] = w[2] - alphas[i] * y_values[index][1] * _mean_loss
    #             w[3] = w[3] - alphas[i] * y_values[index][2] * _mean_loss
    #
    #             _sum = 0
    #             for _temp in range(training_size):
    #                 y_i = w[0] + w[1] * y_values[_temp][0] + w[2] * y_values[_temp][1] + w[3] * y_values[_temp][2]
    #                 _sum += (np.sqrt(np.square(1 / c) * np.square(training_price[_temp] - y_i) + 1) - 1)
    #             loss_func = _sum / training_size
